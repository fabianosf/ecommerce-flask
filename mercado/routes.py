    ...
    if request.method == 'POST':
        ...
        if produto_obj:
            if current_user.compra_disponivel(produto_obj):
               ...
               flash(f'Parabens! Você comprou o produto {produto_obj.nome}', category='success')
            else:
                flash(f'Voce nao possui saldo para comprar o produto{produto_obj.nome}', category='danger')
        ...
        if produto_obj_venda:
            if current_user.venda_disponivel(produto_obj_venda):
                ...
                flash(f'Parabens! Você vendeu o produto {produto_obj_venda.nome}', category='success')
            else:
                flash(f'Algo deu errado! Você não conseguiu realizar a venda do produto {produto_obj_venda.nome}', category='danger')
            return redirect(url_for('page_produto'))
...
def page_logout():
    logout_user()
    flash('Voce fez o logout', category='info')
