...
class CadastroForm(FlaskForm):
    def validate_username(self,check_user):
        ...
        if user:
            raise ValidationError('Usuario ja existente ! Cadastre outro usuario diferente !')
        
    def validate_email(self, check_email):
            email = User.query.filter_by(email=check_email.data).first()
            if email:
                raise ValidationError('Email ja existente ! Cadastre outro email diferente !') 
               
    def validate_senha(self, check_senha):
            senha = User.query.filter_by(senha=check_senha.data).first()
            if senha:
                raise ValidationError('Senha ja existente! Cadastre outra senha')        
